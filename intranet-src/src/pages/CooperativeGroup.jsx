...
function ProjectsTab() {
  ...
  if (selected) return (
    <div>
      <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', color: '#c87533', fontSize: '0.82rem', cursor: 'pointer', padding: 0, marginBottom: '1.25rem', fontFamily: 'inherit' }}>← All Projects</button>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {selected.project_participants.map((pp, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(255,255,255,0.03)', border: '1px solid #1a1a2e', borderRadius: '6px', padding: '0.4rem 0.7rem' }}>
                <span style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(200,117,51,0.15)', color: '#c87533', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.72rem', fontWeight: 700, flexShrink: 0 }}>
    ...
    <div>
      <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        {[['', 'All'], ['active', 'Active'], ['paused', 'Paused'], ['completed', 'Completed']].map(([v, l]) => (
          <button key={v} onClick={() => setStatusFilter(v)} style={{ padding: '0.3rem 0.65rem', background: statusFilter === v ? 'rgba(200,117,51,0.15)' : 'none', border: `1px solid ${statusFilter === v ? 'rgba(200,117,51,0.4)' : '#1a1a2e'}`, color: statusFilter === v ? '#c87533' : '#52526a', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', fontFamily: 'inherit' }}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '0.85rem' }}>
        {filtered.map(m => (
          <div key={m.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.85rem', background: 'rgba(255,255,255,0.025)', border: '1px solid #1a1a2e', borderRadius: '8px', padding: '1rem' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, background: 'rgba(200,117,51,0.12)', color: '#c87533', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', fontWeight: 800 }}>
              {(m.name || '?').charAt(0)}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                ...
                {m.membership_class && <Badge label={MEMBER_CLASS[m.membership_class] || `Class ${m.membership_class}`} color="#c87533" />}
              </div>
            </div>
          </div>
        ))}
...
function GovernanceTab() {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div style={{ display: 'flex', gap: '0.35rem' }}>
          {[['open', 'Open'], ['closed', 'Closed'], ['all', 'All']].map(([v, l]) => (
            <button key={v} onClick={() => setFilter(v)} style={{ padding: '0.3rem 0.65rem', background: filter === v ? 'rgba(200,117,51,0.15)' : 'none', border: `1px solid ${filter === v ? 'rgba(200,117,51,0.4)' : '#1a1a2e'}`, color: filter === v ? '#c87533' : '#52526a', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', fontFamily: 'inherit' }}>
              {l}
            </button>
          ))}
        </div>
        <button onClick={() => setShowNew(!showNew)} style={{ padding: '0.4rem 0.85rem', background: 'rgba(200,117,51,0.12)', border: '1px solid rgba(200,117,51,0.25)', color: '#c87533', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      {/* New proposal form */}
      {showNew && (
        <form onSubmit={submitProposal} style={{ background: 'rgba(200,117,51,0.05)', border: '1px solid rgba(200,117,51,0.15)', borderRadius: '8px', padding: '1.25rem', marginBottom: '1.25rem' }}>
          <div style={{ marginBottom: '0.75rem' }}>
          ...
          <button type="submit" disabled={submitting} style={{ padding: '0.5rem 1.1rem', background: '#c87533', border: 'none', color: '#000', borderRadius: '6px', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
