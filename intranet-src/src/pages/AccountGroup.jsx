...
const TYPE_COLORS = {
  initial: '#c87533', labor: '#50b478', capital: '#4a9eff',
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: '1.5rem' }}>
        <a href="/intranet/documents/" style={{ fontSize: '0.8rem', color: '#c87533', textDecoration: 'none' }}>K-1 documents →</a>
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
        <button
          ...
          style={{ padding: '0.45rem 0.9rem', background: 'rgba(200,117,51,0.15)', border: '1px solid rgba(200,117,51,0.3)', color: '#c87533', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
        >
          {showForm ? 'Cancel' : '+ Log Hours'}
        </button>
        <div style={{ display: 'flex', gap: '0.35rem' }}>
          {['', ...years].map(y => (
            <button key={y || 'all'} onClick={() => setDateFilter(y)} style={{ padding: '0.3rem 0.6rem', background: dateFilter === y ? 'rgba(200,117,51,0.15)' : 'none', border: `1px solid ${dateFilter === y ? 'rgba(200,117,51,0.4)' : '#1a1a2e'}`, color: dateFilter === y ? '#c87533' : '#52526a', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', fontFamily: 'inherit' }}>
              {y || 'All'}
            </button>
          ))}
        </div>
      ...
      {/* Log form */}
      {showForm && (
        <form onSubmit={submit} style={{ background: 'rgba(200,117,51,0.06)', border: '1px solid rgba(200,117,51,0.15)', borderRadius: '8px', padding: '1.25rem', marginBottom: '1.25rem' }}>
          ...
          <button type="submit" disabled={submitting} style={{ padding: '0.5rem 1.25rem', background: '#c87533', border: 'none', color: '#000', borderRadius: '6px', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
...
function PatronageTab({ data }) {
  ...
  return (
    <div>
      <div style={{ fontSize: '0.8rem', color: '#52526a', background: 'rgba(200,117,51,0.06)', border: '1px solid rgba(200,117,51,0.12)', borderRadius: '6px', padding: '0.6rem 0.85rem', marginBottom: '1.25rem' }}>
      ...
      {years.length > 0 && (
        <div style={{ display: 'flex', gap: '0.35rem', marginBottom: '1rem' }}>
          {['', ...years].map(y => (
            <button key={y || 'all'} onClick={() => setYearFilter(y)} style={{ padding: '0.3rem 0.6rem', background: yearFilter === y ? 'rgba(200,117,51,0.15)' : 'none', border: `1px solid ${yearFilter === y ? 'rgba(200,117,51,0.4)' : '#1a1a2e'}`, color: yearFilter === y ? '#c87533' : '#52526a', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', fontFamily: 'inherit' }}>
              {y || 'All'}
            </button>
          ))}
        </div>
    ...
    <div>
      {account && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.85rem', marginBottom: '1.5rem' }}>
          <StatBox label="Book Balance (GAAP)" value={fmtUSD(account.book_balance)} sub="Economic fair value — basis for ownership and redemption" color="#c87533" />
